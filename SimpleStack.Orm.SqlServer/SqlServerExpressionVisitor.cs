...
namespace NServiceKit.OrmLite.SqlServer
{
	...
	public class SqlServerExpressionVisitor<T> : SqlExpressionVisitor<T>
	{
	    ...
	    public override string ToSelectStatement()
        {
	        ...
	        var tableName = Config.DialectProvider.GetQuotedTableName(ModelDef).Trim(); //2
        ...
        /// <summary>Assert valid skip row values.</summary>
	    ...
	    protected virtual string BuildOrderByIdExpression()
	    {
	        ...
	        return String.Format("ORDER BY {0}", Config.DialectProvider.GetQuotedColumnName(ModelDef.PrimaryKey.FieldName));
	    }
