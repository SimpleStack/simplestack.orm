...
namespace SimpleStack.Orm.SqlServer
{
    ...
    public class SqlServerDialectProvider : DialectProviderBase
    {
        ...
        public override string GetDropForeignKeyConstraints(ModelDefinition modelDef)
        {
            ...
            foreach (var fieldDef in modelDef.FieldDefinitions)
            {
                if (fieldDef.ForeignKey != null)
                {
                    ...
                    sb.AppendLine(string.Format("IF EXISTS (SELECT name FROM sys.foreign_keys WHERE name = '{0}')",
                        foreignKeyName));
                    sb.AppendLine("BEGIN");
        ...
        public override string GetStringFunction(string functionName, string quotedColumnName,
        ...
        {
            var sqlQuery =
                @"SELECT *, OBJECTPROPERTY(OBJECT_ID(CONSTRAINT_SCHEMA + '.' + QUOTENAME(CONSTRAINT_NAME)), 'IsPrimaryKey') as IS_PRIMARY_KEY
