...
namespace SimpleStack.Orm.Tests
{
	public abstract partial class ExpressionTests
	{
		...
		public void BulkInsert()
		{
			...
			Console.WriteLine("Inserting 12500 Members using pure ADO.NET and TransactionScope");
			using (var conn = OpenDbConnection())
			...
			Console.WriteLine("Inserting 12500 Members using 'conn.Execute(query,obj) and TransactionScope'");
			using (var conn = OpenDbConnection())
			using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew,TimeSpan.MaxValue))
			{
				conn.CreateTable<Member>(true);
				...
				foreach (var m in members)
				{
					conn.Execute("insert into Member(VAL1,VAL2,VAL3,VAL4,VAL5,VAL6,VAL7,VAL8,VAL9,VAL10) values(@val1,@val2,@val3,@val4,@val5,@val6,@val7,@val8,@val9,@val10)", m, commandTimeout: 300);
				}
				scope.Complete();
				Console.WriteLine("Duration : {0}", DateTime.Now.Subtract(start));
			...
			Console.WriteLine("Inserting 12500 Members using conn.Insert<Member>(member) and TransactionScope");
			using (var conn = OpenDbConnection())
			using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, TimeSpan.MaxValue))
			{
				conn.CreateTable<Member>(true);
			...
			Console.WriteLine("Inserting 12500 Members using 'conn.Execute(query,IEnumerable)' and TransactionScope");
			using (var conn = OpenDbConnection())
			using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, TimeSpan.MaxValue))
			{
				conn.CreateTable<Member>(true);
			...
			Console.WriteLine("Inserting 12500 Members using conn.Insert<Member>(members) and TransactionScope");
			using (var conn = OpenDbConnection())
			using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, TimeSpan.MaxValue))
			{
				conn.CreateTable<Member>(true);
